...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Trito\n'
            + 'ORG:Owner Lolita;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285888258313:+625888258313\n'
            + 'END:VCARD'

//*********** 𝗔𝗣𝗜𝗞𝗘𝗬 ***********//
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa'
const VhtearKey = '1BlnApiIkyPake'
const XteamKey = 'af527965711d015d'
const TobzApi = 'Bidmzz1sJ2L1TKyqaMEU'
const ZeksApi = 'tritoapriansyah'
const LolKey = '4d29c31483e08febf40e2430'
const mankey = 'MAN'
...
⎙ *Lolita BOT*`,jpegThumbnail: fs.readFileSync(`./assets/peler.jpg`)}}}
...
sendButLocation(from, help(prefix), `Lolita BOT ${times}`, fakeimage, [
          {
            buttonId: 'Owner',
            buttonText: {
              displayText: 'OWNER',
...
break
case '#help':
Zitsraa.sendMessage(from, help(prefix),'conversation',{contextInfo: {forwardingScore : 508, isForwarded: true,externalAdReply:{title:'Lolita BOT' ,mediaType:2,thumbnail: fs.readFileSync('./assets/buset.jpg')}},quoted : ftoko})
break
case 'ler':
...
case 'hallo':
             ...
             var sendMsg = await Zitsraa.prepareMessageFromContent(from, { buttonsMessage }, {})
                 ...
                 break
                 case 'bot':
                 pus = 'Lolita BOT Di Sini Ketik .help Untuk Melihat Daftar Menu :)'
                 Zitsraa.sendMessage(from, pus ,text ,{quoted: fgclink})
                 break
                 case 'sbot':
             var buttons = [{ buttonId: 'example20', buttonText: { displayText: 'Self' }, type: 1 }, { buttonId: 'example2', buttonText: { displayText: 'Public' }, type: 1 }]
             var buttonsMessage = { contentText: `STATUS BOT`, footerText: 'Lolita BOT', buttons: buttons, headerType: 1 }
             var sendMsg = await Zitsraa.prepareMessageFromContent(from, { buttonsMessage }, {})
                 Zitsraa.relayWAMessage(sendMsg, { waitForAck: true })
                 break
				case 'segs':
             ...
             var buttonsMessage = { contentText: `Ya Lolita BOT Disini Click Button Di Bawah Untuk Daftar Menu Segs`, footerText: 'Lolita BOT', buttons: buttons, headerType: 1 }
             var sendMsg = await Zitsraa.prepareMessageFromContent(from, { buttonsMessage }, {})
                 Zitsraa.relayWAMessage(sendMsg, { waitForAck: true })
                 break
                  case 'menunsfw':
...
if (!isOwner) return reply('lu owner bukan??')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							Zitsraa.sendMessage(_.jid, buff, image, {caption: `[ _*Lolita BOT- Broadcast*_ ]\n\n${body.slice(4)}`})
						}
...
case 'bc1':
            ...
            
            {
                ...
                if (!cvk.isReadOnly) await Zitsraa.sendText(ids, `[ Lolita BOT BROADCAST ]\n\n${msg}`)
            }
...
case 'animewallpaper':
           ...
           case '.dujin':
                  ...
                  Zitsraa.sendMessage(from, anu, image, {quoted: fgif, caption: fake, thumbnail: tamnel}).catch(e => {
                   ...
                   tamnel = fs.readFileSync('./assets/buset.jpg')
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/pussy`)
                  Zitsraa.sendMessage(from, anu, image, {quoted: fgif, caption: fake, thumbnail: tamnel}).catch(e => {
	               nayz(nyz.error(prefix, command))
                   console.log(e)
	               })
...
caption: hasil, quoted: mek
  ...
  case 'ytsearch':
        	...
        	return await Zitsraa.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								Zitsraa.sendMessage(from, buff, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
						...
						reply(`Kirim video dengan caption ${prefix}sgif atau tag video yang sudah dikirim slurr`)
					}
					break
...
case 'upswtext':
          ...
          case 'help2':
				...
				icatalogReply('daftar menu', 'Lolita BOT')
				
				break
                    case 'asmaulhusna':
                    awok = await fetchJson(`https://megayaa.herokuapp.com/api/random/asmaulhusna`)
                    asma = awok.result
                    husna = `No : ${asma.index}\n`
                    husna += `Latin: ${asma.latin}\n`
                    husna += `Arab : ${asma.arabic}\n`
                    husna += `Indonesia : ${asma.translation_id}\n`
                    husna += `English : ${asma.translation_en}`
                    reply(husna)
                    break
                    case 'textthumb':
case 'text':
